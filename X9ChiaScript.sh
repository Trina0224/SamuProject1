...
#!/bin/bash
CHECKTARGET1=$(df | grep -i /dev/sdd1)
CHECKTARGETARRAY1=($CHECKTARGET1)
RESULT1=${CHECKTARGETARRAY1[5]}

CHECKTARGET2=$(df | grep -i /dev/sda1)
CHECKTARGETARRAY2=($CHECKTARGET2)
RESULT2=${CHECKTARGETARRAY2[5]}

CHECKTARGET3=$(df | grep -i /dev/sdb1)
CHECKTARGETARRAY3=($CHECKTARGET3)
RESULT3=${CHECKTARGETARRAY3[5]}
...
echo "Check environment integrity:"
if [ "$RESULT1" = "/mnt/plotting" ] && [ "$RESULT2" = "/mnt/temp1" ] && [ "$RESULT3" = "/mnt/temp2" ]
then
        echo "PASS"
        ...
        echo "FAIL, please check /dev/sda1, /dev/sdb1 and /dev/sdd1 mounting points."
fi
